...
export default function Projects() {
    ...
    const x = useTransform(scrollYProgress, [0, 1], ["0%", "-38%"]); // Adjust based on how many cards there are
    ...
    return (
        <section ref={targetRef} className="relative h-[175vh] bg-[#0a0a0a] z-20">
            <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
                <div className="w-full max-w-[1400px] mx-auto px-6 md:px-24">
                    <h3 className="text-3xl md:text-5xl font-extrabold tracking-widest text-white mb-16 text-center">
                ...
                <motion.div style={{ x }} className="flex gap-8 px-6 md:px-24 w-max">
                    {projects.map((project, index) => (
                        <div
                            key={index}
                            className="group relative flex flex-col w-[350px] md:w-[450px] shrink-0 rounded-[2rem] overflow-hidden border border-white/10 bg-[#121214] transition-all duration-500 ease-in-out hover:border-cyan-500/60 hover:shadow-[0_0_40px_-5px_rgba(34,211,238,0.35)]"
                        >
                            ...
                            <div className="relative h-64 overflow-hidden bg-black w-full">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center p-6 text-center">
                                    <h4 className="text-3xl md:text-4xl font-bold tracking-tight text-white/90 drop-shadow-lg">{project.title}</h4>
                                </div>
                            </div>

                            {/* Card Content Area */}
                            <div className="p-8 flex flex-col flex-1 border-t border-white/5 relative z-10 bg-[#121214]">
                                ...
                                <div className="flex items-end justify-between mt-auto pt-6 border-t border-white/10 relative">
                                    ...
                                    <a href={project.link} className="w-10 h-10 shrink-0 rounded-full border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-colors group/btn">
                    ...
                    <div className="w-[300px] shrink-0 flex flex-col items-center justify-center px-12 group cursor-pointer">
                        <div className="w-16 h-16 rounded-full border border-white/10 flex items-center justify-center text-white mb-6 group-hover:bg-white/5 transition-colors group-hover:scale-105 duration-300">
                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                        </div>
                        <h4 className="text-2xl font-bold text-white text-center tracking-tight">View All<br />Projects</h4>
                    </div>
                </motion.div>
            </div>
        </section>
